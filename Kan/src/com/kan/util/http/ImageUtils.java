...
public class ImageUtils {
	 ...
	 * 
	 */
	private final static String PHOTOPATH = FileUtils.getSavePath();
	 ...
	 * @throws IOException 
	 */
	public static Bitmap getBitmapFromOnline(String url) throws IOException{
		
		return getBitmapFormOnline(url,-1,-1) ;
	}
	...
	public static Bitmap getBitmapFromLocal(String url, int scaleWidth , int scaleHeight) {
		...
		if(scaleWidth == -1 && scaleHeight == -1){
			 if(width < 500 ){
				 ...
				 float mScaleX = width/500 ;
				 float mHeight = height / mScaleX ;
				 return Bitmap.createScaledBitmap(bitmap, 500, (int)mHeight, true) ;
			 }
		}
		if (bitmap != null) {
			if (scaleX > scaleY) {
	...
	{
		try {
			return PHOTOPATH + URLEncoder.encode(url,"utf-8");
